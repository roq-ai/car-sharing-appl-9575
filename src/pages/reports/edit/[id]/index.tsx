...
import { useRoqClient, useReportFindFirst } from 'lib/roq';
...
function ReportEditPage() {
  ...
  const { data, error, isLoading, mutate } = useReportFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ReportInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.report.update({
        data: values as RoqTypes.report,
        where: {
          id,
        },
      });
