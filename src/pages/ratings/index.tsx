...
import { useRoqClient, useRatingFindManyWithCount } from 'lib/roq';
...
export function RatingListPage(props: RatingListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useRatingFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.rating.delete({ where: { id } });
      await mutate();
    } catch (error) {
